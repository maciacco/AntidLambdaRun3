...
void SystematicsMultitrial(){
  ...
  TFile out("outSys_multitrial_test.root", "recreate");
  std::string outFile{"out_run2_LHC18qr_multitrial_hyperloop_test.root"};//"out_run2_LHC18qr_grid_sys_hyperloop2.root"};

  std::string outFile0{"out_run2_LHC18qr_grid_sys_hyperloop2.root"};//"out_run2_LHC18qr_grid_sys_hyperloop2.root"};
  ...
  for (int i{0}; i < 30; ++i) {
    ...
    for (int iP{1}; iP < 9; ++iP) {
      rhoRatio[i]->SetBinContent(iP, std::abs((rho[i]->GetBinContent(iP)) - rhoRatio[i]->GetFunction("pol0")->GetParameter(0)));
  ...
  for (int i{0}; i < 8; ++i) {
    sys[i] = new TH1D(Form("hsys_%d", i), ";Var;Entries", 200, -0.01, 0.01);
