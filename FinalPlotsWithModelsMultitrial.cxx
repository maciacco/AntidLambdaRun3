...
void FinalPlotsWithModelsMultitrial(){
  gStyle->SetOptStat(0);
  ...
  auto hsys = (TH1D*)fSys->Get("hout");
  TGraphErrors *gOut = (TGraphErrors*)fSys->Get("gout");
  TGraphErrors *gSys = new TGraphErrors();
  for (int i{1}; i < 9; ++i) {
    gSys->AddPoint(hAntidNetL->GetBinCenter(i), hAntidNetL->GetBinContent(i));
    //gSys->AddPoint(hAntidNetL->GetBinCenter(i), gOut->GetPointY(i - 1));
    gSys->SetPointError(gSys->GetN() - 1, 1., hsys->GetBinContent(i));
    //hAntidNetL->SetBinContent(i, gOut->GetPointY(i - 1));
    //hAntidNetL->SetBinError(i, gOut->GetErrorY(i - 1));
  }
  gSys->SetLineWidth(3);
  gSys->SetFillStyle(0);
