...
with open("sys_cuts.txt", "w") as of:
    print(f'cpa\tdcadau\tdcav0pv\tmass\tdcapv\tntpccls\ttpcnsig\ttofnsig\tzvtx', file=of)
    for i in range(50):
        ...
        # print(f'cpa{i+1} {cpa_i}\tdcadau{i+1} {dcadau_i}\tdcav0pv{i+1} {dcav0pv_i}\tmass{i+1} {mass_i}\tdcapv{i+1} {dcapv_i}\tntpccls{i+1} {ntpccls_i}\ttpcnsig{i+1} {tpcnsig_i}\ttofnsig{i+1} {tofnsig_i}\tzvtx{i+1} {zvtx_i}', file=of)
        print(f'{cpa_i}\t{dcadau_i}\t{dcav0pv_i}\t{mass_i}\t{dcapv_i}\t{ntpccls_i}\t{tpcnsig_i}\t{tofnsig_i}\t{zvtx_i}', file=of)
